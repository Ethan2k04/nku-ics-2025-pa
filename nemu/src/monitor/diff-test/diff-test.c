...
void difftest_step(uint32_t eip) {
  ...
  // TODO();
  if (cpu.eax != r.eax) { diff = true; }
  else if (cpu.ebx != r.ebx) { diff = true; }
  else if (cpu.ecx != r.ecx) { diff = true; }
  else if (cpu.edx != r.edx) { diff = true; }
  else if (cpu.esp != r.esp) { diff = true; }
